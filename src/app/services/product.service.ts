...
export class ProductService {
  ...
  public checkProduct(product: Product):Observable<Product>{
    return this.http.patch<Product>(`http://localhost:8089/products/${product.id}`, //Template String ${product.id}
  ...
  saveProduct(product: Product):Observable<Product> {
    return this.http.post<Product>(`http://localhost:8089/products`, //Template String ${product.id}
      product);
  }
